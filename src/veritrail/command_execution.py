...
COLLECTOR_VERSION = "trusted-command/0.2"
    ...
    *,
    ...
    if before is not None:
        if before.fingerprint != resolved.preview["subject_snapshot"]["fingerprint"]:
            raise SafetyError(
                "approved subject snapshot does not match the command Run start"
            )
        ...
        try:
            ...
            diff_counts, final_drift = compare_subject_snapshots(before, after)
        except (OSError, SafetyError, ValidationError):
            collection_errors.append(
