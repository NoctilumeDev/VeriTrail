...
**当前状态：v0 Implementation，M0–M6 FROZEN，M7 PLANNED。** M0 已冻结计划封存、
...
资源、安全和清理终验。M7 已预注册固定四角色配对/反事实合同，但尚未实现；计划编辑、任意
项目命令和完整自举仍是后续里程碑。
...
## M7 预注册四角色配对反事实分析（PLANNED）

M7 候选合同固定 `BASELINE -> TREATMENT -> RESTORED_BASELINE -> NEGATIVE_CONTROL`，只允许
ExperimentPlan 的版本、唯一主要变量值和 seal 变化。它将生成独立 PairedAnalysis，不改写来源
Verdict；定义不代表实现，完整门槛见
...
- [M7 预注册四角色配对反事实分析（PLANNED）](docs/11-m7-preregistered-paired-analysis.md)
