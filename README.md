...
P3 合同曾在状态发布补丁自身的门禁、合入和合入后精确匿名读回全部成立后冻结；后继 P3-A 本地候选实现审查
暴露了 handoff 核验与 Core 路径重读之间的快照连续性缺口。第一份修正 PR #60 又由原始 Python 3.13
`-O` 门禁复现既有 Browser 停止后 route 回调竞态，已关闭且未合并；独立 PR #61 只修复该地基并以
11/11 合入 `main@6e48b5d...`。快照合同修正随后由 PR #62 的原始 11 项门禁通过，以
`main@5d4e7bbbf706d92c98cf36418d6f86b5caf2d3d8` 合入，并由产品 Collector 完成 exact-SHA README/合同
匿名读回。独立 closure 的门禁、合入和合入后读回全部成立后，当时状态恢复为
...
[文档 95](docs/95-p3-core-handoff-freeze-publication.md)。P4 的
[GitHub Evidence Plugin 独立发布合同 0.1](docs/96-p4-github-evidence-release-contract.md)已经完成两轮
...
Acceptance API，而准备矩阵使用的是同版本号的当前源码本地构建；因此 P4 已停止。独立
...
的最后门全部成立后，状态为 `CORE_0.13.0_CONTRACT_FROZEN / C1_NOT_STARTED / P4_BLOCKED`。下一步只能
从新的 exact main 建立 Core 0.13.0 release candidate，不能重制 `v0.12.2` 或继续生成插件发布资产。
...
第一次冻结事实候选 PR #43 被重复出现的 M10 Chromium 清理时序红灯否决并关闭，未合入也未用 rerun
...
[P2 实现与冻结事实](docs/90-p2-public-render-collector-freeze-candidate.md)记录了 fresh anonymous
...
desktop/narrow 两次真实读回。docs-only closure 又经 PR #50 的 11 项门禁合入
`main@2d3877df41d7ec5a3b7b932404f6b622f06862a8`，并从该 exact main 对 README/事实文档完成匿名读回。
最终发布 PR #51 随后被原始 Python 3.10 `-O` 门禁中的 `13.688s > 9s` M10 生命周期反例否决，关闭且
未合并；该红灯没有推翻 P2 实现事实，也没有被旧绿灯或 rerun 覆盖。独立 PR #52 沿完整中断链修正
生命周期与 Chromium 释放预算，在原始 11 项门禁全绿后合入
`main@bb8f2d44c0c681862a5e63506ae57d4b4a8d7947`；修正后的 exact main 又完成匿名 README 与
`RA-018 rev2` Ledger 读回。本状态发布以自身 11 项门禁、受保护主线合入和合入后匿名读回为生效
条件；该链成立后 P2 至此冻结。该时点 P3 实现、P4 与 Review Attention R1 均未开始；P3 的后继重开
状态以下述合同说明为准。
...
不复制 Plan/session 权威、不输出 Verdict。P3-A 本地候选实现已通过纯合同定向回归，但 P3-B 前的
组合审查发现：handoff 与 Core 若先后重读同一路径，两次各自稳定的读取不能证明消费同一快照。合同当时
只重开这一处，要求一次 Core 公共导入形成 `ImportedEvidence`，同一对象再进入通用 Core Bundle 入口；
Schema、evaluator、P1/P2 facts 与 Verdict 优先级均不改变。该修正已经 PR #62、受保护主线与 exact-main
...
P3 已冻结。P4 独立发布合同完成自己的门禁、合入、exact-main 匿名读回与独立冻结发布后，只允许从
新的 exact main 进入发布准备；合同冻结不表示 ruleset、tag、Release 或上传资产已经存在。
