...
冻结后系统复核从 Verdict 归因优先级、Browser 生命周期停止和宿主机资源停止线三个入口继续展开，
并修复所有权、collector/cleanup 事实、阶段断言适用性和压力夹具等关联缺口。`0.11.1.dev1`
补丁又封闭 Windows 瞬时目录锁下的不可变产物原子发布与历史浏览器验收合同漂移，并完成本地发布级
全量复验。首次补丁冻结读回时，GitHub `main` 与 `m10-v0.11.1^{}` 均精确指向实现提交 `f4efdd2`；旧
`m10-v0.11.0` 保持历史寻址且不移动。完整缺陷账册、系统审查与新基线状态见
...
- [M11 真实项目候选适配性与探索记录（HISTORICAL_PROBES）](docs/22-m11-real-project-suitability-and-contract-draft.md)
- [M11 单节点能力与真实项目双门合同 0.2（CONTRACT_FROZEN / IMPLEMENTATION_NOT_STARTED）](docs/23-m11-single-node-real-project-contract.md)
