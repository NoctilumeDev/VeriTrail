...
并修复所有权、collector/cleanup 事实、阶段断言适用性和压力夹具等关联缺口。当前 `0.11.1.dev1`
候选已完成本地发布级全量复验，并在严格冻结轮继续修复 Windows 瞬时目录锁下的不可变产物原子
发布与历史浏览器验收合同漂移；旧 `m10-v0.11.0` 保持历史寻址且不移动。新补丁标签的 GitHub
读回仍是最后门禁；完整缺陷账册、系统审查与新基线状态见
