...
| Review Attention / R | 人应该优先看哪些源码关系与切片？ | Review Artifact / Attention Proposal | SourceSnapshot 已冻结；Derivation Input Binding 合同候选正在冻结前审查 |
...
| Review / R | 确定性源码事实、语义切片与未来注意力提案 | `R1_SOURCE_SNAPSHOT_FROZEN / R1_DERIVATION_INPUT_BINDING_CONTRACT_CANDIDATE / R1_DERIVATION_INPUT_IMPLEMENTATION_NOT_STARTED / R1_FACT_RELATION_SLICE_COVERAGE_IMPLEMENTATION_NOT_STARTED` |
...
- Review Attention 已冻结 R0、首个 Pattern Corpus 与 R1 合同。R1 首版只针对 Python 3.10 结构语义，
  ...
  [Derivation Input Binding 合同候选](docs/132-r1-derivation-input-binding-contract.md)。该候选只约束三份
  Artifact 与 exact Git bytes 的输入连续性；实现尚未开始，parser、Facts、Relations、Slices、Coverage
  与完整 Derivation Manifest 仍未启动。
