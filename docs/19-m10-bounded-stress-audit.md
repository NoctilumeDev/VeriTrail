...
因此阶段 D 当时标记为 `STRESS_AUDITED`。这个状态只证明 M10 在预注册的 16 GB Windows/C1、有界微并行
与回环只读负载内守住所有权、隔离、Evidence、Verdict、资源和清理不变量；它不证明正式通用并行、
生产吞吐、其他平台、第二种真实项目或 M11 能力。阶段 E 随后完成最终候选回归、系统/代码/安全
整改、发布级内置浏览器与残留门禁；`0084443` 和 `m10-v0.11.0^{}` 已从 GitHub 读回一致，M10
当前为 `FROZEN`。本文件的压力结论边界不因里程碑冻结而扩大。
