...
证据工作台也已分别独立冻结。M4 本地 Run 目录与轻量自举已达到 `AUTOMATED`：SQLite、
本地 API、目录一致性、UI 与两阶段自举候选已经实现并取得候选运行证据，但仍等待干净提交上的
最终复跑与冻结。执行编排、跨 Run 比较和完整自举仍是
v0 验收合同，不是已完成清单。每个里程碑只有代码、自动化和适用的真实运行证据齐全后，
才能独立冻结。
