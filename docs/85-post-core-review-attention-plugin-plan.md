...
已建立该 runtime value、二十二格合同证据与受保护主线实现基线；[最终冻结发布](135-r1-derivation-input-freeze-publication.md)
外部绑定候选门禁、合入、exact-main 门与匿名读回。其自身最后门成立后，Input Binding runtime 才为
`R1_DERIVATION_INPUT_FROZEN`；所有后继派生对象继续未开始。

Input Binding 冻结后的[前置审计](136-r1-derivation-attempt-and-fact-provenance-audit.md)没有把 parser 当作
自然下一步，而是先暴露 exact request provenance、Provider applicability、execution budget 与
ProviderRun/Fact 双向闭包仍未冻结。[文档 137](137-r1-derivation-attempt-and-fact-provenance-contract.md)
据此只建立 `R1_DERIVATION_PROVENANCE_CONTRACT_CANDIDATE`：首个适用域严格限制为一个显式 required
`python-ast` binding，用 shared budget context 与可替换确定性测试 Provider 证明 application-owned Fact
identity。现有 Evidence Schema 还需独立增加 memory/artifact budget diagnostics 并禁止非成功 run 报告
Fact/Relation IDs。[文档 138](138-r1-derivation-provenance-contract-freeze-publication.md)只在候选本地复验、
PR #117 原始门禁、受保护主线合入、exact-main 门与已安装产品匿名读回成立后发布
`R1_DERIVATION_PROVENANCE_CONTRACT_FROZEN`。该冻结只允许后继独立修正 Schema/corpus；该修正冻结、
budget primitive feasibility 与明确 runtime 授权成立以前，真实 parser 与所有后继对象仍未获授权。

后继[文档 139](139-r1-derivation-evidence-schema-correction-audit.md)复现旧 Schema 会接受非成功 run/overall
携带 canonical IDs，同时拒绝两个必要 budget code，并确认已经公开冻结的 `0.1 path + $id + bytes` 不能
原位改写。[文档 140](140-r1-derivation-evidence-schema-correction-contract.md)因此只建立
`DerivationEvidence 0.1.1` 补丁合同：旧十个 Schema 与 corpus 原字节保留，新版本只增加 typed diagnostic、
非成功 reported-ID 空集与独立 correction vectors。候选经 PR #119 原始门禁、受保护主线合入、exact-main
门与匿名读回后，由[文档 141](141-r1-derivation-evidence-schema-correction-contract-freeze-publication.md)发布
`R1_DERIVATION_EVIDENCE_SCHEMA_CORRECTION_CONTRACT_FROZEN`，只允许后继物化合同限定的
Schema/corpus/test。该 correction 独立冻结后仍必须先完成 budget primitive feasibility 和明确 runtime 授权。
