...
> 状态：`CONTRACT_FROZEN / SERIAL_VALIDATED`
...
> 目标冻结标签：`m10-v0.11.0`
> 实施门禁：公共退出矩阵、地基审查与严格串行轮已完成，状态为 `SERIAL_VALIDATED`；16 GB 压力轮仍未开始
...
[M10 第一轮严格串行完整复验](18-m10-serial-validation.md)；16 GB 压力轮和发布门禁仍未完成。
