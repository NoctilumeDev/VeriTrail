...
> 冻结后地基纠偏版本：Python Core `0.11.1.dev1`，Workbench `0.11.1-dev.1`；见文档 21
> 历史冻结标签：`m10-v0.11.0`（不移动）
> 当前补丁标签：`m10-v0.11.1` @ `f4efdd2`（不移动）
> 实施门禁：补丁发布门禁全部通过；首次读回时 GitHub `main` 与 `m10-v0.11.1^{}` 均指向 `f4efdd2`
