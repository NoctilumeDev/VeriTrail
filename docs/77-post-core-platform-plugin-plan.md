...
| 层 | 唯一职责 | 明确不负责 |
| --- | --- | --- |
| GitHub | 外部仓库、提交、PR、检查、Release、Pages 与公开页面的原始状态 | VeriTrail 计划、断言或裁决 |
...
状态、可见标记、链接目标和内容签名。API 事实与浏览器事实是并列证据源，不能互相冒充。
...
5. Core 0.12.2、M0–M14、E0–E3 以及全部标签均未移动；
6. 文档通过链接、空白、敏感信息与历史口径检查；
7. P0 设计经受保护主线合入并完成公开读回。
