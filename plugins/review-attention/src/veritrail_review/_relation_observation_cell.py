...
from veritrail_review._relation_observation_application import (
    ...
    build_relation_observation_request_document,
    validate_relation_observation_terminal_document,
)
...
class _PreparedRelationObservationAttempt:
    def __init__(
        self,
        *,
        ...
        self.observation_domain = copy.deepcopy(dict(observation_domain))
...
def build_prepared_relation_observation_attempt(
    inputs: DerivationInputSet,
    *,
    derivation_id: str,
    binding: ProviderBinding,
    ...
    fact_set_document: Mapping[str, object],
    observation_domain: Mapping[str, object],
) -> _PreparedRelationObservationAttempt:
    if (
        ...
        or not relation_observation_binding_matches_allow_list(binding)
        ...
        or not isinstance(observation_domain, Mapping)
    ):
    ...
    try:
        request_document = build_relation_observation_request_document(
            inputs=inputs,
    ...
    return _PreparedRelationObservationAttempt(
        inputs=inputs,
        derivation_id=derivation_id,
        binding=binding,
...
def run_prepared_relation_observation_attempt(
    prepared: _PreparedRelationObservationAttempt,
) -> OwnedRelationObservationCellPhaseResult:
    if not isinstance(prepared, _PreparedRelationObservationAttempt):
        raise DerivationExecutionCellError(
            DerivationExecutionCellFailureCode.INVALID_DERIVATION_ATTEMPT_REQUEST
        )
    ...
    expected_run_id = provider_run_id(
        request_document["derivation_id"],
        prepared.binding.descriptor,
        request_document["operands_digest"],
    )
    ...
    if observation.stop_trigger is not None:
        ...
        try:
            ...
            terminal = validate_relation_observation_terminal_document(
                untrusted_terminal, request=request_document
            )
        except (FrameProtocolError, RelationObservationApplicationError):
            eligibility.revoke()
            status = _internal_failure()
        else:
    ...
    return OwnedRelationObservationCellPhaseResult(
        ...
        observation_domain_digest=request_document[
            "observation_domain_digest"
        ],
        assigned_observation_item_ids=tuple(
            request_document["assigned_observation_item_ids"]
        ),
        ...
        observation_outcome_bytes=outcome_bytes,
