...
PREVIOUS_STABLE_CORE_VERSION = "0.12.2"
STABLE_CORE_VERSION = "0.13.0"
...
CORE_0_13_0_RELEASE_READBACK = (
    REPOSITORY_ROOT / "docs" / "106-core-v0.13.0-release-readback-facts.md"
)
...
class CoreVersionContractTests(unittest.TestCase):
    def test_source_uses_the_released_0_13_0_coordinate(self) -> None:
        ...
        self.assertNotEqual(project_version, PREVIOUS_STABLE_CORE_VERSION)
    ...
    def test_current_release_and_historical_coordinates_are_not_conflated(self) -> None:
        ...
        self.assertIn("当前 Latest Core 已由后述 0.13.0 发布闭环更新", agents)
        self.assertIn("docs/106-core-v0.13.0-release-readback-facts.md", agents)
        self.assertRegex(
            contributing,
            r"Core `0\.13\.0` is the current released\s+stable coordinate",
        )
        self.assertRegex(
            security,
            r"Core 0\.13\.0 is the current released stable\s+coordinate",
        )
        self.assertNotRegex(
            agents,
        ...
        self.assertIn("CORE_0.13.0_RELEASED", agents)
        self.assertIn("latest publicly released Core coordinate is `0.13.0`", contributing)
        self.assertIn("protected tag, five Release assets", security)
        ...
        self.assertIn(
            "[![Core v0.13.0](https://img.shields.io/badge/Core-v0.13.0-0B4B50)]"
            "(https://github.com/NoctilumeDev/VeriTrail/releases/tag/v0.13.0)",
            readme,
        )
        self.assertIn(
            "| 直接使用稳定内核 | [Core 0.13.0]"
            "(https://github.com/NoctilumeDev/VeriTrail/releases/tag/v0.13.0) |",
            readme,
        )
        self.assertIn(
        ...
        self.assertNotIn("[![Core v0.12.2]", readme)
        self.assertNotIn("| 直接使用稳定内核 | [Core 0.12.2]", readme)
        self.assertIn(
        ...
        self.assertIn(
            "> 当前稳定内核：[`VeriTrail Core 0.13.0`]"
            "(https://github.com/NoctilumeDev/VeriTrail/releases/tag/v0.13.0)",
            start_here,
        )
        self.assertIn(
            "releases/download/v0.13.0/veritrail-0.13.0-py3-none-any.whl",
            start_here,
        )
        self.assertNotIn(
            "> 当前稳定内核：[`VeriTrail Core 0.12.2`]", start_here
        )
        self.assertNotIn(
        ...
        self.assertIn(
            ...
            "(106-core-v0.13.0-release-readback-facts.md)",
            milestones,
        )

        self.assertIn(CURRENT_SOURCE_VERSION, contributing)
        self.assertIn(
            "latest publicly released Core coordinate is `0.13.0`", contributing
        )
        self.assertNotIn("source version is not a public install coordinate", contributing)
        ...
        release_0_13_0 = CORE_0_13_0_RELEASE_NOTES.read_text(encoding="utf-8")
        readback_0_13_0 = CORE_0_13_0_RELEASE_READBACK.read_text(encoding="utf-8")
        ...
        self.assertRegex(
            release_0_13_0,
            r"(?m)^> 状态：`RELEASED / PUBLIC READBACK COMPLETE`$",
        )
        self.assertRegex(
            release_0_13_0,
            r"(?m)^> 当前公开稳定 Core：`0\.13\.0`$",
        )
        self.assertRegex(
            release_0_13_0,
            r"(?m)^> 发布源码：`main@cd6f85246c0a789a3117861144af95deeb1b7077`$",
        )
        self.assertIn("CORE_0.13.0_PUBLIC_READBACK_COMPLETE", readback_0_13_0)
        self.assertIn(
            "a03c7a1fbcff88291c8cb86f2718c8d76d1ed709", readback_0_13_0
        )

        bug_template = BUG_TEMPLATE.read_text(encoding="utf-8")
        self.assertIn("placeholder: v0.13.0 or 40-character commit SHA", bug_template)
        self.assertNotIn("placeholder: v0.12.2 or 40-character commit SHA", bug_template)

        bounded_proposal = BOUNDED_PROPOSAL_TEMPLATE.read_text(encoding="utf-8")
        self.assertIn(
            "Core v0.12.0/v0.12.1/v0.12.2/v0.13.0 or historical entry-layer releases",
            bounded_proposal,
        )
    ...
    def test_starter_020_rejects_unsupported_core_0130(self) -> None:
