...
- 当前阶段是 `v0 Implementation`。M0 已在提交 `64497779add1351014d802b38d46f73a4ce394ac`
  ...
  M8“预注册全因子批次矩阵与固定种子扰动”已完成 Python Core 首个切片：四个公共 Schema、
  `seal-batch` / `analyze-batch`、脱敏 2×2 示例和自动化反例已 `IMPLEMENTED / AUTOMATED`；
  Workbench、真实 M5 批次、Codex 内置浏览器终验与冻结仍为 `PENDING`，当前不得标记为
  `FROZEN` 或开启 M9 实现。
